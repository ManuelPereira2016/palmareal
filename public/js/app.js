...
window.searchPage = function () {
  ...
  _createClass(searchPage, [{
    key: 'getMedia',
    value: function getMedia(media, propertyId) {
    ...
    value: function search() {
      ...
      __WEBPACK_IMPORTED_MODULE_0_axios___default.a.post(window.location.href.split('?')[0], data).then(function (res) {
        res = res.data;
        var row = $('<div class="row"></div>');
        var length = res.properties.length;

        // Check if we need to remove flash message
        !res.message ? $('#message').removeClass('alert-info').text('').hide() : '';

        res.properties.map(function (prop) {
          var img = res.media.filter(function (item) {
            return _this.getMedia(item, prop.id);
          });

          row.append(_this.properties(prop, img[0]));
    ...
    key: 'getPages',
    value: function getPages(url) {
      var _this2 = this;

      window.showLoading('#properties-container');

      __WEBPACK_IMPORTED_MODULE_0_axios___default.a.post(url, this.filters).then(function (res) {
        res = res.data;
        var row = $('<div class="row"></div>');

        res.properties.map(function (prop) {
          var img = res.media.filter(function (item) {
            return _this2.getMedia(item, prop.id);
          });
          row.append(_this2.properties(prop, img[0]));
    ...
    key: 'triggerScrollTop',
    value: function triggerScrollTop() {
      $('html, body').animate({
        scrollTop: $('#properties-page').offset().top
      }, 1000);
    }
  }, {
    key: 'runEventsListeners',
    value: function runEventsListeners() {
      var _this3 = this;

      $('#properties-container').on('click', '.pagination a', function (e) {
        e.preventDefault();
        var url = $(e.target).attr('href');
        window.history.pushState("", "", url);
        _this3.triggerScrollTop();
        _this3.getPages(url);
      });
